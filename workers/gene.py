...
def compute_qc(self, *args, **kwargs):
    ...
    filename,requested_genes,use_cached,no_compute = args
    self.update_state(state="PROGRESS", meta={"position": "preparation" , "progress" : 0})
    if not no_compute:
        download_path = tempDir.joinpath(filename)
        computed_filename= Path(download_path).parent.joinpath(Path(download_path).stem+"_computed.h5ad").__str__()
        adata=None
        if Path(computed_filename).exists() and use_cached:
        ...
        out['clusters_number']=list(map(lambda x: utils.ToNumber(x), out['clusters']))
...
@app.task(bind=True)
def compute_qc_liya(self, *args, **kwargs):
    config=utils.load_configuration()
    aws_s3=utils.AWS_S3(config)
